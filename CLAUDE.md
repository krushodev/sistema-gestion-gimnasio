...
5. **Catálogos seed-only e identidad visual (16/09/2026):** Provincia, Localidad, TipoDocumento, MedioPago, Rol, Plan y Maquina dejaron de tener ABM en la UI — vienen sembrados con la aplicación (`docs/SGIG_CreateDB.sql`, sección 9) y solo se consultan. `frmTablasParametricas` (y sus 5 editores) se eliminó del todo; `frmPlanes`/`frmMaquinas` quedaron de solo lectura. Detalle completo en `docs/patrones/catalogos-seed-only.md`. Los 3 roles del sistema siguen fijos (Administrador/Recepcionista/Técnico); lo único que era realmente dinámico y se mantiene es la asignación de uno de esos 3 roles al dar de alta un `Usuario`. En paralelo se agregó `SGIG.UI/Tema.cs`, un helper de estilo compartido (colores/fuentes ya usados en `frmLogin`/`frmMDIParent`) para que toda la app tenga la misma identidad visual — ver `docs/patrones/estilo-visual.md` para la convención y por qué no toca ningún `.Designer.cs`.

Las primeras tres notas no obligaron a tocar la ERS: el RNF#08 solo pide "C# sobre Visual Studio", sin especificar versión de .NET ni técnica de acceso a datos. La cuarta sí, y por eso la ERS pasó a v4.0 antes de que se tocara una línea de código. La quinta tampoco toca la ERS: es una decisión de alcance de la UI (qué catálogos tienen ABM), no del modelo de datos ni de los roles del sistema.
...
- **Transacciones (`SqlTransaction`)** para cualquier alta que toque más de una tabla: alta de Persona+Socio o Persona+Usuario, alta de Mantenimiento (inserta el mantenimiento y cambia el estado de la Máquina, RF#20), y el registro de un cobro de cuota (crea la `Facturacion`, inserta el `Pago` y actualiza `Socio.fecha_vencimiento_cuota`, RF#11/RF#12). Con Dapper la transacción se abre igual que con ADO.NET puro y se pasa explícitamente a cada llamada:
  ...
  ```
- **Baja lógica**, nunca DELETE físico, sobre `Socio` y `Usuario`. Confirmar siempre con `MessageBox` Sí/No antes (RNF#03). `Plan` ya no tiene baja desde la UI — ver nota histórica #5: es un catálogo sembrado, de solo lectura.
